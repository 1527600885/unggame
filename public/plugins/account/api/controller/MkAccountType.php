...
// +----------------------------------------------------------------------
namespace plugins\account\api\controller;

use app\api\BaseController;
use plugins\account\api\model\MkAccountType as MkAccountTypeModel;
/**
 * MkAccountType管理
 */
class MkAccountType extends BaseController
{
    /**
     * 显示资源列表
     * 访问：test.com/api/account/mkAccountType/index
     */
    public function index()
    {
        if ($this->request->isPost()) {
            ...
            $count = MkAccountTypeModel::count();
            $data  = MkAccountTypeModel::page($page, 10)->select();
    ...
    {
        if ($this->request->isPost()) {
            MkAccountTypeModel::find(input("post.id"));
