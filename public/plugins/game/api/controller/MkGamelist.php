...
use plugins\game\api\model\MkGamelist as MkGamelistModel;
/**
 * MkGamelist管理
 */
class MkGamelist extends BaseController
{
    /**
     * 显示资源列表
     * 访问：game.uswindltd.com/api/game/mkGamelist/index
     */
    public function index()
    {
        if ($this->request->isPost()) {
            ...
            $count = MkGamelistModel::count();
            $data  = MkGamelistModel::page($page, 10)->select();
    ...
    {
        if ($this->request->isPost()) {
            MkGamelistModel::find(input("post.id"));
    ...
    {
        if ($this->request->isPost()) {
            MkGamelistModel::update(input("post."));
