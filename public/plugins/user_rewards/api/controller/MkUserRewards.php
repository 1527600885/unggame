...
// +----------------------------------------------------------------------
namespace plugins\user_rewards\api\controller;

use app\api\BaseController;
use plugins\user_rewards\api\model\MkUserRewards as MkUserRewardsModel;
/**
 * MkUserRewards管理
 */
class MkUserRewards extends BaseController
{
    /**
     * 显示资源列表
     * 访问：test.com/api/user_rewards/mkUserRewards/index
     */
    public function index()
    {
        if ($this->request->isPost()) {
            ...
            $count = MkUserRewardsModel::count();
            $data  = MkUserRewardsModel::page($page, 10)->select();
    ...
    {
        if ($this->request->isPost()) {
            MkUserRewardsModel::find(input("post.id"));
