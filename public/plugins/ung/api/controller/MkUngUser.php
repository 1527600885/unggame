...
use plugins\ung\api\model\MkUngUser as MkUngUserModel;
/**
 * MkUngUser管理
 */
class MkUngUser extends BaseController
{
    /**
     * 显示资源列表
     * 访问：www.unicgm.com/api/ung/mkUngUser/index
     */
    public function index()
    {
        if ($this->request->isPost()) {
            ...
            $count = MkUngUserModel::count();
            $data  = MkUngUserModel::page($page, 10)->select();
    ...
    {
        if ($this->request->isPost()) {
            MkUngUserModel::find(input("post.id"));
