...
use plugins\user\api\model\MkUserIdcard as MkUserIdcardModel;
/**
 * MkUserIdcard管理
 */
class MkUserIdcard extends BaseController
{
    /**
     * 显示资源列表
     * 访问：test.com/api/user/mkUserIdcard/index
     */
    public function index()
    {
        if ($this->request->isPost()) {
            ...
            $count = MkUserIdcardModel::count();
            $data  = MkUserIdcardModel::page($page, 10)->select();
    ...
    {
        if ($this->request->isPost()) {
            MkUserIdcardModel::find(input("post.id"));
