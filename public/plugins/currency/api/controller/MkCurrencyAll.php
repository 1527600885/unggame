...
// +----------------------------------------------------------------------
namespace plugins\currency\api\controller;

use app\api\BaseController;
use plugins\currency\api\model\MkCurrencyAll as MkCurrencyAllModel;
/**
 * MkCurrencyAll管理
 */
class MkCurrencyAll extends BaseController
{
    /**
     * 显示资源列表
     * 访问：test.com/api/currency/mkCurrencyAll/index
     */
    public function index()
    {
        if ($this->request->isPost()) {
            ...
            $count = MkCurrencyAllModel::count();
            $data  = MkCurrencyAllModel::page($page, 10)->select();
    ...
    {
        if ($this->request->isPost()) {
            MkCurrencyAllModel::find(input("post.id"));
