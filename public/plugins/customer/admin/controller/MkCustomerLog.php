...
class MkCustomerLog extends BaseController
{
    ...
    public function index()
    {
        if ($this->request->isPost()) {
            $input = input("post.");
            $count = MkCustomerLogModel::withSearch(["keyword"], $input)->count();
            $data  = MkCustomerLogModel::withSearch(["keyword"], $input)->order($input["prop"], $input["order"])->page($input["page"], $input["pageSize"])->select();
            foreach($data as $k=>$v){
				if($v->online_time){
