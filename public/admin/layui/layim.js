...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var layimMain, popim = function(content){
    return layer.open({
      ...
      ,success: function(layero){
        ...
        setSkin(layero);
        if(cache.base.left){
          layero.css('margin-left', cache.base.left);
        }
        if(layimClose){
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<img src="'+ layui.cache.dir+'logo.jpg' +'">'
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
        ...
        ,content: cache.base.chatLog + '?to=' + thatChat.data.id + '&type=' + thatChat.data.type + "&mine=" + cache.mine.id
      });
    }
