	    	...
	    	try{
		        ...
		        if(count($userdataid)>0){
			        	...
			        	foreach ($userdataid as $key => $value) {
			        		$isexists = $redis->hExists('ung_user_divd:ung_user_'.$value);
			        		if($isexists){
			        			...
			        			if($ungdata['num']>0 && time()<$ungdata['divd_time']+3600){
								    ...
								    Db::commit();
								    $redis->hSet('ung_user_divd:ung_user_'.$ung_data['uid'],'divd_time',time());
			        			}
			        		}else{
			        			// 从数据苦衷写入redis
		        	...
		        	// 从数据苦衷写入redis
		        	$ung_data = Db::name('ung_user')->where()->select()->toArray();
		        	foreach ($ung_data as $key => $value) {
		        		$redis->hSet('ung_user_divd:ung_user_'.$ung_data['uid'],'num',$ung_data['num']);
			            $redis->hSet('ung_user_divd:ung_user_'.$ung_data['uid'],'update_time',time());
			            $redis->sAdd('ung_user_id',$ung_data['uid']);
		        	}
		        }
